...
void view_goods() {
    ...
    while (fscanf(file, "%s %s %s %s %f %d %d", g.code, g.name, g.category, g.unit, &g.price, &g.min_stock, &g.max_stock) != EOF) {
...
void view_categories() {
    ...
    char category[50];
    printf("\n当前分类列表：\n");
    while (fscanf(file, "%s", category) != EOF) {
        printf("- %s\n", category);
    }
    fclose(file);
}
...
void view_stock() {
    ...
    // 读取并显示库存信息
    while (fscanf(file, "%s %d %s %s %s", s.goods_code, &s.quantity, s.location, s.batch_no, s.expire_date) != EOF) {
        printf("%s\t%d\t%s\t%s\t%s\n", s.goods_code, s.quantity, s.location, s.batch_no, s.expire_date);
    }
...
void view_records() {
    ...
    while (fscanf(file, "%s %s %d %d %s %s %[^\n]", r.record_id, r.goods_code, &r.quantity, 
                   &r.type, r.operator, r.date, r.note) != EOF) {
...
//     int found = 0;
//     long pos;
//     while ((pos = ftell(file)) >= 0 && fscanf(file, "%s %d", location, &occupied) != EOF) {
//         if (!occupied) {
//             found = 1;
//             printf("为商品 [%s] 分配库位：%s\n", goods_code, location);
...
void turnover_rate_analysis() {
    ...
    while (fscanf(file, "%s %s %d %d %s %s %[^\n]", r.record_id, r.goods_code, &r.quantity, &r.type, r.operator, r.date, r.note) != EOF) {
        if (r.type == 1) {
